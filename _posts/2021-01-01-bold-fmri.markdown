...
Visualize the correlation map as an overlay on the T1 using *afni*. Increase the threshold to find the brain area of maximum activation, and show each subject's correlation map overlayed on the T1 in *afni* with a threshold of 0.15. Show 16 rows of images with each subject's correlation map overlayed on the T1 (axial view):
